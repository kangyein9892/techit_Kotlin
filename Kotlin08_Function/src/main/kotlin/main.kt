...
fun main(){
    ...
    // 매개변수에 기본값이 설정되어 있는 함수 호출 a2와 a3에는 기본값이 설정되어 있지만 함수 호출시 값을 전달하고 있기 때문에 기본값은 무시되고 전달해주는 값이 저장된다.
...
// 기본값을 가지고 있는 매개변수에 기본값이 설정되어 있는 경우
...
fun test4(){
    ...
    // 함수 내부에 코드가 더 있다고 하더라도 함수의 수행을 끝내고 함수를 호출한 쪽으로 돌아가라는 명령어
    // 개발자가 만든 코드가 비정상적으로 동작할 상황이라면 코드가 동작되지 않도록 하기 위해 return문을 사용한다.
    return
