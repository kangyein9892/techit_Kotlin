...
fun main(){
    ...
    // 부모클래스를 작성할 당시에는 자식 클래스가 어떤 것들이 있는지 알수도 없고 부모클래스 작성 코드에는 자식클래스가 무엇인지 명시하는 곳이 없기 때문에 객체를 생성할 때 자식클래스 부분을 생성할 수 없다.
    // 오류
    // println(s2.SubA2)
    // s2.subMethod2()
...
class SubClass2 : SuperClass2{
    constructor(){
        // 부모클래스의 생성자를 명시적으로 호출하지 않으면 부모 클래스가 가지고 있는 생성자 중 매개변수가 없는 생성자를 자동으로 호출한다.
    }
